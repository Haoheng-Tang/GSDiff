'''This is used to train a node generation model with sloping walls.'''
...
sys.path.append('/root/autodl-tmp/house_diffusion-main')
sys.path.append('/root/autodl-tmp/house_diffusion-main/datasets')
sys.path.append('/root/autodl-tmp/house_diffusion-main/house_diffusion')
import math
import torch
import shutil
...
from datasets.rplang_edge_semantics_simplified_55_100_82 import RPlanGEdgeSemanSimplified_55_100_82
from datasets.rplang_edge_semantics_simplified_82 import RPlanGEdgeSemanSimplified_82
from house_diffusion.heterhouse_75_106 import HeterHouseModel
from house_diffusion.heterhouse_56_13 import EdgeModel
...
'''create output_dir'''
output_dir = 'outputs/structure-82-106/'
os.makedirs(output_dir, exist_ok=False)
'''record description'''
description = '''
非曼哈顿假设，方法是对于属于这种情况的阳台：有三条边的两侧分别是阳台和外侧，剩下一条边的两侧是阳台和非外侧的其他类型
将与“阳台和非外侧的其他类型”平行的那条边的两个端点向着那条边的中点对称移动，将边长缩短到原来的0.618
'''
...
'''Data'''
dataset_train = RPlanGEdgeSemanSimplified_55_100_82('train')
...
dataset_val = RPlanGEdgeSemanSimplified_55_100_82('val')
dataloader_val = DataLoader(dataset_val, batch_size=batch_size_val, shuffle=False, num_workers=64,
                        drop_last=False, pin_memory=False)  # try different num_workers to be faster
dataloader_val_iter = iter(cycle(dataloader_val))

dataset_val_for_gt_rendering = RPlanGEdgeSemanSimplified_82('val')
...
while step < total_steps:
    ...
    if step % interval == 0:
        ...
        for batch_count in tqdm(range(batch_numbers)):
            corners_withsemantics_0_val_batch, global_attn_matrix_val_batch, corners_padding_mask_val_batch = next(dataloader_val_iter)
        ...
        # model 2 loading
        model_path_2 = 'outputs/structure-56-34/' + 'model_stage2_best_013100.pt'
        ...
        for val_count in tqdm(range(len(dataset_val))):
            ...
            with torch.no_grad():
                ...
                output_edges_val, _, _ = model_2(corners_stage2_val, global_attn_matrix_stage2_val, corners_padding_mask_stage2_val, semantics_stage2_val)
        ...
        for k_val in [0]:
            ...
            for val_count in tqdm(range(len(dataset_val))):
                ...
                d_rev_val, simple_cycles_val, simple_cycles_semantics_val = get_cycle_basis_and_semantic_3_semansimplified(output_points_val, output_edges_val)
                # for k, v in d_rev_val.items():
                #     print(k, v)
                # for sc in simple_cycles_val:
                #     print(sc)

'''This is used to test (generate) 3000 samples with sloping walls.'''
...
'''create output_dir'''
output_dir = 'test_outputs/Non-manhattan7/'
...
'''Data'''
dataset_test = RPlanGEdgeSemanSimplified_55_100_82('test')
dataloader_test = DataLoader(dataset_test, batch_size=batch_size_test, shuffle=False, num_workers=128,
                        drop_last=False, pin_memory=False)  # try different num_workers to be faster
dataloader_test_iter = iter(cycle(dataloader_test))

dataset_test_for_gt_rendering = RPlanGEdgeSemanSimplified_82('test')
...
model_path_EdgeModel = 'outputs/structure-56-82-interval1000/model_stage2_best_075000.pt'
...
# 扩散模型
test_metrics = []
model_path_CDDPMs = ['outputs/structure-82-106/' + fn for fn in os.listdir('outputs/structure-82-106') if 'model' in fn and '.pt' in fn]
for model_path_CDDPM in model_path_CDDPMs:
    if int(model_path_CDDPM[5 + len('outputs/structure-82-106/'):8 + len('outputs/structure-82-106/')]) % 1000 == 100: # model1000000.pt
        ...
        for batch_count in tqdm(range(batch_numbers)):
            '''a batch of data'''
            corners_withsemantics_0_test_batch, global_attn_matrix_test_batch, corners_padding_mask_test_batch = next(

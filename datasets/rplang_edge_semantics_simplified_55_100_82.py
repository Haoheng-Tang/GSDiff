...
class RPlanGEdgeSemanSimplified_55_100_82(Dataset):
    '''
   ...
   to facilitate the data loading. we don't use the order in nn, to meet permutation invariability of graph nodes.)
    ...
    def __getitem__(self, index):
        ...
        '''attn 1 matrix, (53, 53)'''
        global_attn_matrix_4seman = graph['global_matrix_np_padding'].astype(bool)
        ...
        global_attn_matrix_temp_4seman = global_attn_matrix_4seman[None, :, :]
        ...
        edges_temp_depadded = edges[None, :, :][global_attn_matrix_temp_4seman.reshape(1, -1, 1)][None, :, None]
        ...
        if bal_hot.sum() == 4:
            ...
            if len(index_six_count) == 3 and len(non_six_count) == 1:
                ...
                p1 = ((c1 * 0.191 + c2 * 0.809) - 128) / 128
                p2 = ((c2 * 0.191 + c1 * 0.809) - 128) / 128
